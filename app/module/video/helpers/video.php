...
class video {
	...
	static public function genre ( $id, $type = 'none' ) {
	...
	static public function collection ( $id, $type = 'none' ) {
		$option = url::url2params();
		$option = url::url2appli($option);

		$a = array();
		$a['select'] = 'video.*';
		$a['from'][] = 'collectioner collectioner';
		$a['where'][] = 'collectioner.video_id = video.id';
		$a['where'][] = 'collectioner.collection_id = '.$id;

		if ( $type != 'count' ) $a['limit'] = $option['limit'];
		$a['orderby'] = 'collectioner.order ASC';

		$Video = new VideoModel;
		return $Video->listes( $a, $type );
	}

	static public function lang ( $id, $type = 'none' ) {
		$option = url::url2params();
		$option = url::url2appli($option);
	...
	static public function artist ( $id, $type = 'none' ) {
		$option = url::url2params();
		$option = url::url2appli($option);
	...
	static public function director ( $id, $type = 'none' ) {
		$option = url::url2params();
		$option = url::url2appli($option);
	...
	static public function actor ( $id, $type = 'none' ) {
		$option = url::url2params();
		$option = url::url2appli($option);
	...
	static public function date ( $d, $type = 'none' ) {
		$option = url::url2params();
		$option = url::url2appli($option);
